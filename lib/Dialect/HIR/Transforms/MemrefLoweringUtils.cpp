...
void emitCallOpOperandsForMemrefPort(
  ...
  auto sendAttr = helper::getDictionaryAttr(helper::getHIRBusPortAttrName(),
                                            builder.getStrArrayAttr({"send"}));
  auto recvAttr = helper::getDictionaryAttr(helper::getHIRBusPortAttrName(),
...
std::string toString(MemKindEnum memKind) {
  switch (memKind) {
  case MemKindEnum::reg:
    return "reg";
  case MemKindEnum::bram:
    return "bram";
  case MemKindEnum::lutram:
    return "lutram";
  }
  assert(false && "Unknown MemKind");
}

std::string createHWMemoryName(MemKindEnum memKind, hir::MemrefType memrefTy,
                               ArrayAttr memPorts) {
  std::string name =
      toString(memKind) + "_" +
...
std::string createVerilogMemoryName(MemKindEnum memKind, ArrayAttr memPorts) {
  std::string name = toString(memKind);
  for (auto port : memPorts) {
    name += "_";
    if (helper::isMemrefRdPort(port))
      name +=
...
LogicalResult emitMemoryInstance(OpBuilder &builder, hir::MemrefType memrefTy,
