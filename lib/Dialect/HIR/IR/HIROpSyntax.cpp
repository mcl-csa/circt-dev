...
ParseResult CallOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  if (parser.resolveOperands(operands, funcTy.getInputTypes(),
                             parser.getNameLoc(), result.operands))
    return failure();
  if (parser.resolveOperand(tstart, helper::getTimeType(context),
                            result.operands))
    return failure();
  ...
  result.addTypes(funcTy.getResultTypes());
  return success();
}

void CallOp::print(OpAsmPrinter &printer) {
  printer << " ";
  printer << this->instance_nameAttr();
  printer << " @" << this->callee();
  printer << "(" << this->operands() << ") at ";
...
// ParseResult CallInstanceOp::parse(OpAsmParser &parser, OperationState
// &result) {
...
//      parser.resolveOperands(operands, funcTy.getInputTypes(), argLoc,
//                             result.operands))
//    return failure();
//  if (parser.resolveOperand(tstart, helper::getTimeType(context),
//                            result.operands))
...
//}
//
// void CallInstanceOp::print(OpAsmPrinter &printer) {
//  printer << "hir.call.instance " << this->callee();
//  printer << "(" << this->operands() << ") at ";
//  printTimeAndOffset(printer, this->getOperation(), this->tstart(),
//                     this->offsetAttr());
//  printer << " : ";
//  printer << this->callee().getType();
//}
