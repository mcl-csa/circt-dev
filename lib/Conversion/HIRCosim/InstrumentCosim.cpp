...
// Helper functions.
static void removeHIRAttrs(std::function<void(StringAttr attrName)> removeAttr,
                           ArrayRef<NamedAttribute> attrList) {
...
LogicalResult CPUModuleBuilder::visitOp(hir::ProbeOp op) {
  ...
  builder.create<mlir::func::CallOp>(
      op.getLoc(), TypeRange(),
      SymbolRefAttr::get(builder.getStringAttr("record")), op.input());
  op.erase();
  return success();
}
